...
const HH_API_USER_AGENT = "job-bot/1.0 (youremail.com)";
...
const formatSalary = (salary?: HhApiSalary | null): string | undefined => {
  ...
  if (typeof salary.from === "number") {
    parts.push(`от ${salary.from}`);
  }

  if (typeof salary.to === "number") {
    parts.push(`до ${salary.to}`);
  }
...
export const mapApiVacancy = (item: HhApiVacancy): NormalizedVacancy => ({
  id: item.id,
  title: item.name?.trim() || "Не указано",
  company: item.employer?.name?.trim() || "Не указано",
